...
class SharedBuffer:
    ...
    def __init__(self, config: VexConfig, manager=None):
        ...
        self.inference_queue = manager.Queue() # worker gpu puts inference requests (worker ids). 
        self.res_events = [manager.Event() for _ in range(config.n_workers)] # set by infrence gpu when results are ready. 
        self.read_write_queue = manager.Queue() # worker pull from queue to write, trainer gpu pull from queue to read. ensures no overlapping write/read.
...
class SharedLeague:
    ...
    def __init__(self, config: VexConfig, manager=None):
        ...
        del local_model
        self.latest_idx = manager.Value('i', 0) # index of the latest snapshot in the bank.
        self.global_counter = manager.Value('i', 0) # used to get unique id for each snapshot version.
        self.param_bank = torch.zeros((self.max_league_snapshots, total_params), dtype=torch.float32).pin_memory().share_memory_() # store model parameters of league snapshots.
        self.param_version = torch.zeros((self.max_league_snapshots,), dtype=torch.long).pin_memory().share_memory_() 
        self.qualities = torch.full((self.max_league_snapshots,), float('-inf'), dtype=torch.float32).pin_memory().share_memory_() # store quality of league snapshots. higher is better.
        self.qualities[self.latest_idx.value] = 0 
        self.lock = manager.Lock()
        
        self.learner_param = torch.zeros((total_params,), dtype=torch.float32).pin_memory().share_memory_()
        self.learner_updated_event = manager.Event() # set by trainer gpu when new learner has been added 
        self.learner_lock = manager.Lock() 
    ...
    def update_latest_snapshot(self, model: AgentMLP):
        ...
        with self.lock:
            ...
            self.qualities[replace_idx].copy_(torch.tensor(new_quality, dtype=torch.float32))
            self.latest_idx.value = replace_idx
            self.param_bank[replace_idx].copy_(param)
            self.param_version[replace_idx].copy_(torch.tensor(self.global_counter.value, dtype=torch.long))
